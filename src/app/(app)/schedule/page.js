...
import { SchedulePrintDoc } from "./schedule-print-doc";
    ...
    <>
        ...
        {!classId ? (
          ...
          <>
            ...
            <div className="space-y-3">
              <PrintActions
                label={`Programme de ${selectedClass?.name ?? ""}`}
                fileTitle={`Emploi du temps - ${selectedClass?.name ?? ""}`}
              />
              {view === "week" ? (
      ...
      {classId ? (
        <SchedulePrintDoc
          ...
          yearLabel={currentYear?.label ?? "non définie"}
          view={view}
          slots={slots}
